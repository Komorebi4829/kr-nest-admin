...
class XLilKm1715347323708 implements typeorm.MigrationInterface {
    name = 'XLilKm1715347323708'
    ...
    public async up(queryRunner: typeorm.QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `CREATE TABLE \`role_menus\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL COMMENT '菜单名称', \`type\` enum ('0', '1', '2') NOT NULL COMMENT '菜单类型' DEFAULT '0', \`icon\` varchar(255) NULL COMMENT '菜单图标', \`customOrder\` int NOT NULL COMMENT '显示排序' DEFAULT '0', \`isFrame\` tinyint NULL COMMENT '是否外链', \`isCache\` tinyint NULL COMMENT '是否缓存', \`path\` varchar(255) NOT NULL COMMENT '路由地址', \`component\` varchar(255) NULL COMMENT '组件路径', \`perms\` varchar(255) NULL COMMENT '权限字符', \`query\` varchar(255) NULL COMMENT '路由参数', \`visible\` tinyint NULL COMMENT '显示状态', \`status\` tinyint NULL COMMENT '菜单状态', \`mpath\` varchar(255) NULL DEFAULT '', \`parentId\` int NULL, FULLTEXT INDEX \`IDX_6176fb9d4dc4968663f473b260\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        )
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE \`content_tags\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL COMMENT '分类名称', \`description\` varchar(255) NULL COMMENT '标签描述', FULLTEXT INDEX \`IDX_6f504a08a58010e15c55b1eb23\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        )
        await queryRunner.query(
            `CREATE TABLE \`content_posts\` (\`id\` varchar(36) NOT NULL, \`title\` varchar(255) NOT NULL COMMENT '文章标题', \`body\` text NOT NULL COMMENT '文章内容', \`summary\` varchar(255) NULL COMMENT '文章描述', \`keywords\` text NULL COMMENT '关键字', \`type\` enum ('html', 'markdown') NOT NULL COMMENT '文章类型' DEFAULT 'markdown', \`publishedAt\` varchar(255) NULL COMMENT '发布时间', \`customOrder\` int NOT NULL COMMENT '自定义文章排序' DEFAULT '0', \`createdAt\` datetime(6) NOT NULL COMMENT '创建时间' DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL COMMENT '更新时间' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL COMMENT '删除时间', \`categoryId\` varchar(36) NULL, \`authorId\` varchar(36) NOT NULL, FULLTEXT INDEX \`IDX_9ef6db9d13df6882d36c8af0cc\` (\`title\`), FULLTEXT INDEX \`IDX_e51068c39974ca11fae5d44c88\` (\`body\`), FULLTEXT INDEX \`IDX_f43723dc196c18767a3893a3f7\` (\`summary\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        )
        await queryRunner.query(
            `CREATE TABLE \`content_categories\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL COMMENT '分类名称', \`customOrder\` int NOT NULL COMMENT '分类排序' DEFAULT '0', \`mpath\` varchar(255) NULL DEFAULT '', \`parentId\` varchar(36) NULL, FULLTEXT INDEX \`IDX_d6aaf8517ca57297a8c3a44d3d\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        )
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE \`role_menus\` (\`id\` varchar(36) NOT NULL, \`label\` varchar(255) NOT NULL COMMENT 'i18n 标签', \`name\` varchar(255) NOT NULL COMMENT '菜单名称', \`type\` enum ('0', '1', '2') NOT NULL COMMENT '菜单类型' DEFAULT '0', \`icon\` varchar(255) NULL COMMENT '菜单图标', \`customOrder\` int NOT NULL COMMENT '显示排序' DEFAULT '0', \`isFrame\` tinyint NULL COMMENT '是否外链', \`frameSrc\` varchar(255) NULL COMMENT '外链地址', \`isCache\` tinyint NULL COMMENT '是否缓存', \`path\` varchar(255) NOT NULL COMMENT '路由地址', \`component\` varchar(255) NULL COMMENT '组件路径', \`perms\` varchar(255) NULL COMMENT '权限字符', \`query\` varchar(255) NULL COMMENT '路由参数', \`hide\` tinyint NULL COMMENT '显示状态', \`status\` tinyint NULL COMMENT '菜单状态', \`newFeature\` tinyint NULL COMMENT '是否新特性', \`hideTab\` tinyint NULL COMMENT '是否隐藏tab', \`mpath\` varchar(255) NULL DEFAULT '', \`parentId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        )
        await queryRunner.query(
            `ALTER TABLE \`role_menus\` ADD CONSTRAINT \`FK_875b128ce419d8a668f62727588\` FOREIGN KEY (\`parentId\`) REFERENCES \`role_menus\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
        )
    ...
    public async down(queryRunner: typeorm.QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `ALTER TABLE \`role_menus\` DROP FOREIGN KEY \`FK_875b128ce419d8a668f62727588\``,
        )
        await queryRunner.query(
        ...
        await queryRunner.query(`DROP INDEX \`IDX_6176fb9d4dc4968663f473b260\` ON \`role_menus\``)
...
module.exports = XLilKm1715347323708
