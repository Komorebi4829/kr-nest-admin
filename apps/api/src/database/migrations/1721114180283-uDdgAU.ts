...
class UDdgAU1721114180283 implements typeorm.MigrationInterface {
    name = 'UDdgAU1721114180283'
    ...
    public async up(queryRunner: typeorm.QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`operation_logs\` (\`id\` varchar(36) NOT NULL, \`operation_name\` varchar(255) NOT NULL COMMENT '操作的名称或描述', \`operation_type\` enum ('create', 'update', 'delete', 'read', 'other') NOT NULL COMMENT '操作类型（创建、读取、更新、删除、其它）', \`operation_time\` datetime NULL COMMENT '操作时间', \`operation_ip\` varchar(40) NULL COMMENT '操作IP', \`operation_device\` varchar(500) NULL COMMENT '设备信息如浏览器类型,操作系统等', \`operation_url\` varchar(255) NULL COMMENT '请求URL', \`status\` enum ('success', 'failed') NULL COMMENT '操作状态', \`fail_reason\` varchar(500) NULL COMMENT '登录失败原因', \`time\` varchar(20) NULL COMMENT '执行时间', \`method\` enum ('GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS', 'HEAD') NULL COMMENT '请求方法', \`userId\` varchar(36) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`)
...
module.exports = UDdgAU1721114180283
