...
import { genDocPath } from '@/bootstrap/routes'

import { BaseRestful } from '@/helpers/BaseClass'
import {
    ApiConfig,
    APIDocOption,
    ApiDocSource,
...
export class Restful extends BaseRestful {
    ...
    async setupSwagger<T extends INestApplication>(
        container: T,
    ...
    protected createDocs() {
        ...
        const vDocs = versionMaps.map(([name, version]) => [name, this.getDocOption(name, version)])
